...
class PolicyTranslator:
	...
	def __init__(self, fileNamePrefix, gen=False,mdpLoad = False,qLoad = False,humObs = True,alphaLoad = None):
		...
		#Grab Modeling Code
		allMod = D2DiffsModel(fileNamePrefix); 
	...
	def ani(self,bels,allX0,allX1,allX2,allX3,numFrames = 20):
		...
		my_ani.save("../Results/animation.gif",fps = 1)
		#plt.show(); 
...
if __name__ == "__main__":
	...
	alLoad = '../policies/DiffsAlphas2.npy'; 
	...
	********
	...
	a = PolicyTranslator(fileNamePrefix = fileNamePrefix,gen = generate,mdpLoad = mdpPolicy,qLoad = qmdp,humObs = hObs,alphaLoad = alLoad); 
	...
	if(sim):
		if(simRand):
			...
			if(greedySim):
				fileSave = '../Results/diffs/' + fileNamePrefix + '_Data_Greedy' + str(dataNumber) + '.npy';
			else:
			...
			for j in range(0,len(a.delA[0])):
				inPose[j] = random.random()*a.bounds[j][1] + a.bounds[j][0];  
			...
			for b in allB:
				...
				ax3.scatter(allX[allB.index(b)][0],allX[allB.index(b)][1],c='r'); 
				plt.pause(0.1); 
