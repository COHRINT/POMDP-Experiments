...
class InterceptTestGenerator:
		...
		for j in range(0,len(G)):
			for a in range(0,len(self.delA)):
				for o in range(0,len(self.soft_weight)):
					...
					alObs = G[j].runVB(self.soft_weight,self.soft_bias,self.soft_alpha,self.soft_zeta_c,modelNum = o); 
	...
	def buildObs(self):
		self.soft_weight = [0,15,25];  
		self.soft_bias = [0,-20,-45];
	...
	def beliefUpdate(self,b,a,o,maxMix = 10):
		...
		btmp1 = GM(); 
		for j in b.Gs:
			mean = (np.matrix(j.mean) + np.matrix(self.delA[a])).tolist(); 
			var = (np.matrix(j.var) + np.matrix(self.delAVar)).tolist(); 
			weight = j.weight; 
		btmp = btmp1.runVB(self.soft_weight,self.soft_bias,self.soft_alpha,self.soft_zeta_c,modelNum = o); 
...
if __name__ == "__main__":
	...
	alsave = '../policies/softInterceptAlphas1.npy'; 
