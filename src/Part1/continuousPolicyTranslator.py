...
class continuousPolicyTranslator():
	...
	def getNextPose(self,pose,cop,randMoves = False):
		...
		if(cop):
			if(self.tag):
				o = 0; 
				if(self.distance(x1,y1,x2,y2) < 2):
					o = 5; 
				elif(self.distance(x1,y1,4.5,4.5)<3.5):
					o = 4; 
				...
				self.beliefUpdate(action,o,[destX1,destY1]); 
			else:
	...
	def setTagInitialBelief(self,pose = [0,0]):
		self.B = GM(); 
		self.B.addG(Gaussian([pose[0],pose[1],4.5,4.5],np.eye(4)*6,1)); 
	...
	def beliefUpdate(self,a,o,pose):
		...
		'''
		#Adds a belief at the cops actual state
		g = Gaussian(); 
		g.mean = [pose[0],pose[1],4.5,4.5]; 
		g.var = (np.eye(4)*200).tolist(); 
		g.var[0][0] = 0.0001; 
		g.var[1][1] = 0.0001; 
		g.weight = 100; 
		btmp.addG(g); 
	...
	def simulate(self):
		...
		while(self.distance(x1,y1,x2,y2) >= 1.4):
			...
			#[xx,yy,cc] = self.B.marginalizeTo2DFrom4D(low=[0,0],high=[10,10],res = 20);
			#print(self.B.getMeans()); 
	...
	def simBeliefs(self,steps,fileName = "cTagBeliefs5.npy",randMoves = False):
		...
		'''
		while(count < steps):
			beliefs[count] = self.B; 
			count = count + 1; 

			if(randMoves):
				cop = self.getNextPose([x1,y1,x2,y2],True,randMoves = randMoves); 
				rob = self.getNextPose([x1,y1,x2,y2],False,randMoves = randMoves);
			else:
				cop = self.getNextPose([x1,y1,x2,y2],True); 
				rob = self.getNextPose([x1,y1,x2,y2],False)
			x1 = cop[0]; 
		...
		for i in range(0,3):
			for j in range(0,3):
				for k in range(0,3):
					for l in range(0,3):
						tmp = GM(); 
						tmp.addG(Gaussian([i*3,j*3,k*3,l*3],np.eye(4).tolist(),1)); 
						beliefs.append(tmp); 
		f = open(fileName,"w"); 
		np.save(f,beliefs); 
...
if __name__ == "__main__":
	c = continuousPolicyTranslator(fileName = "../policies/cTagAlphas6.npy",hardware = False,tag = True,initBelief = [0,0]); 
	c.simulate(); 
	#c.simBeliefs(50,randMoves = True); 
